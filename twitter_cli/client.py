...
def _chrome_emulation_for_target(target):
    ...
    return Emulation(profile=profile, platform=Platform.Linux, headers=True)
...
def _get_wreq_session():
    ...
    if _wreq_session is None:
        ...
        sync_chrome_version(target)  # align UA/sec-ch-ua with emulation target
        client = _WreqClient(
        ...
        logger.info("wreq emulating %s", target)
        if proxy:
...
class TwitterClient:
    ...
    def upload_media(self, file_path):
        ...
        headers = self._build_headers(url=upload_url, method="POST")
        # Remove JSON content-type so wreq can send form encoding.
